  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const filePromises = Array.from(files).map((file) => {
        return new Promise<{ url: string; isImage: boolean }>((resolve, reject) => {
          const reader = new FileReader();
          if (file.type.startsWith('image/')) {
            reader.onload = (e) => resolve({ url: e.target?.result as string, isImage: true });
            reader.readAsDataURL(file);
          } else {
            reader.onload = () => resolve({ url: '', isImage: false });
      ...
      const newImages = results.filter((r) => r.isImage).map((r) => r.url);
      ...
      if (newImages.length > 0) {
        ...
        if (searchable) {
          ...
          setSnackbar({ open: true, message: 'Images imported and added to the search database!', severity: 'success' });
        }
      }

      if (results.some((r) => !r.isImage)) {
        setSnackbar({ open: true, message: 'Files imported successfully!', severity: 'success' });
      }
    } catch (error) {
      console.error('Import failed', error);
      setSnackbar({ open: true, message: 'Import failed. Please try again.', severity: 'error' });
    } finally {
      setLoading(false);
          ...
          >
            {dbImages.map((img, idx) => (
              ...
              >
                <img
                  src={img}
                  alt={`Database collectible ${idx + 1}`}
                  loading="lazy"
                  style={{ maxWidth: '100%', maxHeight: '100%' }}
                />
              </Box>
