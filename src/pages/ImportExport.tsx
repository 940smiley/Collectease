...
export default function ImportExport() {
  ...
  // Lazy initialization prevents reading from LocalStorage on every render
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Parallelize file reading for better performance using Promise.all
    const filePromises = Array.from(files).map((file) => {
      return new Promise<string | null>((resolve) => {
        if (file.type.startsWith('image/')) {
          ...
          const reader = new FileReader();
          reader.onload = (e) => {
            console.log(`Imported file "${file.name}" contents:`, e.target?.result);
            resolve(null);
          };
    ...
    if (newImages.length > 0) {
      ...
      if (searchable) {
        // Optimization: Save only new images to the search database and update state
        // using the returned value to avoid a redundant LocalStorage read.
        const updated = saveImagesToSearchDB(newImages);
        setDbImages(updated);
