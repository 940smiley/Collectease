          ...
          reader.onload = (e) => {
            ...
            if (loaded === files.length) {
              ...
              if (searchable) {
                ...
                setSnackbar({
                  open: true,
                  message: `${newImages.length} images imported and added to the search database!`,
                  severity: 'success',
                });
  ...
  return (
    <Paper elevation={2} sx={{ p: 4 }}>
      ...
      <FormControlLabel
        ...
        sx={{ mb: 2 }}
      />
      <Tooltip title="Upload images or data files (CSV, JSON, TXT) to your collection" arrow>
        <Button
          variant="contained"
          startIcon={<UploadFileIcon />}
          onClick={() => fileInputRef.current?.click()}
          sx={{ mt: 2 }}
          aria-label="Import collection files or images"
        >
      ...
      <Tooltip title="Download the image search database as a JSON file" arrow>
        <span>
          <Button
            variant="outlined"
            sx={{ mt: 4 }}
