...
import { Typography, Paper, Snackbar, Alert, CircularProgress, type AlertColor } from '@mui/material';
...
export default function ImportExport() {
    ...
    try {
      ...
      if (newImages.length > 0) {
        ...
        if (searchable) {
          ...
          setSnackbarMessage(`${newImages.length} images imported and added to search database!`);
          setSnackbarOpen(true);
        } else {
          setSnackbarSeverity('success');
          setSnackbarMessage(`${newImages.length} images imported!`);
          setSnackbarOpen(true);
        }
      } else {
        setSnackbarSeverity('info');
        setSnackbarMessage('Files imported! (See console for contents)');
        setSnackbarOpen(true);
      }
    } catch (error) {
      setSnackbarSeverity('error');
      setSnackbarMessage('Failed to import files.');
      setSnackbarOpen(true);
  ...
  const handleExport = () => {
    try {
      ...
      setSnackbarSeverity('success');
      setSnackbarMessage('Search database exported successfully!');
      setSnackbarOpen(true);
    } catch (error) {
  ...
  return (
    <Paper elevation={2} sx={{ p: 4 }}>
      ...
      <Button
        ...
        disabled={isLoading}
      >
        {isLoading ? 'Importing...' : 'Import Files or Images'}
      </Button>
      <input
        type="file"
